...
func (s *TodoService) UploadTodoAttachment(
	...
	src, err := file.Open()
	if err != nil {
		logger.Error().Err(err).Msg("failed to open the uploaded file")
		return nil, errs.NewBadRequestError("failed to open the uploaded file", false, nil, nil, nil)
	}
	defer src.Close()
