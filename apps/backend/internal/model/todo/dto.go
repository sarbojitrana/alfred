...
type GetTodosQuery struct{
	...
	Sort			*string			`query:"sort" valiate:"omitempty,oneof=created_at updated_at title priority due_date status"`
	Order			*string			`query:"order" validate:"omitempty,oneof=asc dec"`
...
type GetTodoByPayload struct{
	ID		uuid.UUID 			`param:"id" validate:"required,uuid"`
}

func (p *GetTodoByPayload) Validate() error{
	validate := validator.New()
	return validate.Struct(p)
}
...
type DeleteTodoByPayload struct{
	ID		uuid.UUID 			`param:"id" validate:"required,uuid"`
}

func (p *DeleteTodoByPayload) Validate() error{
	validate := validator.New()
	return validate.Struct(p)
}
